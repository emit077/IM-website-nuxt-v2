...
import CityImageIcon from '~/components/contact/CityImageIcon.vue'
import { alsoServing, branchOffices, popularCityImages } from '~/data/contact'

const selectedCityId = ref<string | null>(null)

const popularCityNames = new Set(
  popularCityImages.flatMap((city) => [city.label.toLowerCase(), city.id.toLowerCase()]),
)
...
const selectedPopularCity = computed(() =>
  popularCityImages.find((city) => city.id === selectedCityId.value) ?? null,
)
  ...
  >
    <div class="container-page">
      ...
      <div class="mt-12">
        ...
        <div
          class="mt-6 -mx-4 flex gap-3 overflow-x-auto px-4 pb-3 sm:mx-0 sm:flex-wrap sm:justify-center sm:gap-x-6 sm:gap-y-8 sm:overflow-visible sm:px-0"
          role="list"
        >
          <button
            v-for="city in popularCityImages"
            :key="city.id"
            type="button"
            role="listitem"
            :aria-pressed="selectedCityId === city.id"
            :class="[
              'group flex w-[92px] shrink-0 flex-col items-center gap-2.5 rounded-xl px-1 py-2 transition duration-200 sm:w-[100px]',
              selectedCityId === city.id
                ? 'bg-white ring-1 ring-blue-200 shadow-soft'
                : 'hover:bg-white/70',
            ]"
            @click="toggleCity(city.id)"
          >
            <CityImageIcon :image="city.image" :label="city.label" />
            <span
              :class="[
        ...
        >
          <div
            v-if="selectedOffice"
            class="mx-auto mt-2 max-w-2xl rounded-2xl border border-slate-200/80 bg-white px-5 py-4 text-center shadow-soft sm:px-6 sm:py-5"
          >
      ...
      <div id="city-images-other" class="scroll-mt-24 mt-14 sm:mt-16">
        <h3 class="text-center text-lg font-semibold text-slate-700 sm:text-xl">
          Other Cities
        </h3>
