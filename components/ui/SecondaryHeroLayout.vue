...
const tickerItems = computed(() => {
  const items = props.heroContent.ticker ?? []
...
<template>
  <section class="secondary-hero relative overflow-x-clip" :aria-labelledby="showTitle ? headingId : undefined">
    ...
    <slot name="ticker">
      <div v-if="tickerItems.length" class="relative z-[4] -mb-5 h-[4.5rem] sm:-mb-6 sm:h-20"
        :aria-label="heroContent.tickerAriaLabel || 'Highlights'">
        <div
          class="theme-ticker-strip absolute inset-x-[-6%] top-0 origin-center rotate-[2.5deg] scale-[1.06] py-3 shadow-[0_8px_24px_-12px_rgba(0,0,0,0.35)] sm:py-3.5">
          <div class="overflow-hidden">
            <div
              class="secondary-hero-marquee-reverse flex w-max items-center gap-7 motion-reduce:animate-none sm:gap-9">
...
.secondary-hero-marquee-reverse {
  animation: secondary-marquee-reverse 40s linear infinite;
}

@keyframes secondary-marquee-reverse {
  from {
