...
import { tutorWhyChoose } from '~/data/tutors'
</script>

<template>
  <section id="why-tutors-choose" class="relative scroll-mt-28 overflow-hidden section-surface-muted section-py"
    aria-labelledby="why-tutors-choose-heading">
    <div class="container-page relative">
      <CardHeader heading-id="why-tutors-choose-heading" :badge="tutorWhyChoose.badge" :title="tutorWhyChoose.title"
        :classes="tutorWhyChoose.classes" />

      <ul class="mt-10 grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4" role="list">
        <li v-for="(item, i) in tutorWhyChoose.items" :key="item.title" v-motion :initial="{ opacity: 0, y: 14 }"
