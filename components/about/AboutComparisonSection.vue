...
<template>
  <section
    id="comparison"
    class="relative overflow-hidden bg-[#f8fafc] py-14 sm:py-16 lg:py-20"
  ...
  >
    ...
    <div class="container-page relative">
      ...
      <div
        class="mx-auto mt-10 grid max-w-4xl grid-cols-1 gap-3 sm:grid-cols-3 sm:gap-4"
        v-motion
      ...
      >
        ...
        >
          ...
          <p :class="['font-display mt-3 text-sm font-bold sm:text-base', col.highlight ? 'text-white' : 'text-slate-900']">
            {{ col.label }}
          </p>
          ...
          <p :class="['text-[11px] font-medium uppercase tracking-wider', col.highlight ? 'text-blue-100/80' : 'text-slate-400']">
            features matched
          </p>
        </div>
      </div>
      ...
      <!-- Desktop table -->
      <div
        class="mx-auto mt-8 max-w-4xl overflow-hidden rounded-[1.75rem] border border-slate-200/80 bg-white shadow-soft"
        v-motion
      ...
      >
        <div class="hidden overflow-x-auto lg:block">
          <table class="w-full min-w-[720px] border-collapse text-left text-sm">
            ...
            <tbody>
              <tr
                v-for="(row, ri) in aboutComparison.rows"
                :key="row.feature"
                :class="[
              ...
              >
                <td class="px-6 py-4 font-medium text-slate-800">
                  <span class="flex items-center gap-3">
                    <span
                      class="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-slate-100 text-[11px] font-bold tabular-nums text-slate-500"
                      aria-hidden="true"
                    >
      ...
      <p
        class="mx-auto mt-8 max-w-xl text-center text-sm leading-relaxed text-slate-500"
        v-motion
        :initial="{ opacity: 0 }"
