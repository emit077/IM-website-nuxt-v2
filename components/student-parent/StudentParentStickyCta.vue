...
import { spStickyCta } from '~/data/student-parent'

const { y } = useWindowScroll()
const { height: viewportHeight } = useWindowSize()
const dismissed = ref(false)

/** Hidden near the page bottom so the bar never covers the footer. */
...
<template>
  <Transition enter-active-class="transition duration-300 ease-out"
    ...
    <div v-show="visible"
      ...
      role="region" aria-label="Student enrollment plan actions">
      <div class="container-page flex items-center gap-3 py-3 sm:gap-5 sm:py-3.5">
        ...
        <div class="flex flex-1 items-center gap-2.5 sm:flex-none">
          <a :href="spStickyCta.freeHref"
            ...
            {{ spStickyCta.freeLabel }}
          </a>
          <a :href="spStickyCta.premiumHref"
            ...
            <Icon icon="mdi:star-four-points-outline" class="h-4 w-4 shrink-0" aria-hidden="true" />
            {{ spStickyCta.premiumLabel }}
          </a>
        </div>
        ...
        <button type="button"
          ...
          aria-label="Dismiss plan actions" @click="dismissed = true">
